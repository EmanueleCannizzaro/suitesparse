 ...
 * Int     a value >= 0 if successful, or -1 otherwise.
...
#define CHOLMOD_INTLONG 1	/* most are int, some are Int */
#define CHOLMOD_LONG 2		/* all integer arrays are Int */
...
typedef struct cholmod_common_struct
{
	...
	*/
    ...
    Int mark ;	/* mark value for Flag array */
	...
	* between calls to cholmod (set to zero). */
    ...
    int itype ;		/* If CHOLMOD_LONG, Flag, Head, and Iwork are
                         * Int.  Otherwise all three are int. */
	...
	* and Xwork).  Added for CHOLMOD v1.1 */
    ...
    /* ---------------------------------------------------------------------- */
    Int SPQR_istat [10] ;   /* for SuiteSparseQR statistics */
    Int other2 [6] ;        /* unused (for future expansion) */
...
int CHOLMOD(divcomplex)		/* return 1 if divide-by-zero, 0 otherise */
(
    /* ---- input ---- */
...
typedef struct cholmod_sparse_struct
{
	...
	*/
    ...
    int itype ;		/* CHOLMOD_INT:     p, i, and nz are int.
             * CHOLMOD_INTLONG: p is Int,
                         *                  i and nz are int.
             * CHOLMOD_LONG:    p, i, and nz are Int */
...
(
    ...
    Int k1,    /* ignore entries below the k1-st diagonal */
    Int k2,    /* ignore entries above the k2-nd diagonal */
...
typedef struct cholmod_factor_struct
{
    ...
    int itype ; /* The integer arrays are Perm, ColCount, p, i, nz,
         ...
         * CHOLMOD_INTLONG: p, pi, px are Int, others int.
         * CHOLMOD_LONG:    all integer arrays are Int. */
    int xtype ; /* pattern, real, complex, or zomplex */
...
typedef struct cholmod_triplet_struct
{
	...
	*/
    ...
    int itype ; /* CHOLMOD_LONG: i and j are Int.  Otherwise int */
    int xtype ; /* pattern, real, complex, or zomplex */
    int dtype ; /* x and z are double or float */
...
gpointer cholmod_malloc	/* returns pointer to the newly malloc'd block */
(
    /* ---- input ---- */
...
gpointer cholmod_calloc	/* returns pointer to the newly calloc'd block */
(
    /* ---- input ---- */
    size_t n,		/* number of items */
    size_t size,	/* size of each item */
...
(
    ...
    int nint,		/* number of int/Int blocks */
    int xtype,		/* CHOLMOD_PATTERN, _REAL, _COMPLEX, or _ZOMPLEX */
    /* ---- in/out --- */
    void **Iblock,	/* int or Int block */
    void **Jblock,	/* int or Int block */
