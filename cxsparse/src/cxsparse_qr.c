...
cs_n *cs_qr (const cs *A, const cs_s *S)
{
    Real *Rx, *Vx, *Ax, *x ;
    double *Beta ;
    ...
    x = cs_malloc (m2, sizeof (Real)) ;           /* get Real workspace */
    N = cs_calloc (1, sizeof (cs_n)) ;               /* allocate result */
    if (!w || !x || !N) return (cs_ndone (N, NULL, w, x, 0)) ;
    s = w + m2 ;                                    /* s is size n */
    for (k = 0 ; k < m2 ; k++) x [k] = 0 ;          /* clear workspace x */
    N->L = V = CXSPARSE(spalloc) (m2, n, vnz, 1, 0) ;      /* allocate result V */
    N->U = R = CXSPARSE(spalloc) (m2, n, rnz, 1, 0) ;      /* allocate result R */
